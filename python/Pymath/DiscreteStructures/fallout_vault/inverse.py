...
class Inverse:
    def compute_inverse(self):
        ...
        try:
            ...
            Label(self.frame_inverse_output,
                  text="Invalid input(s)").grid(row=2, column=1)

        try:
            list_mat = [str(i) for i in (inv(self.matrix))]
            return list_mat
        except:
            Label(self.frame_inverse_output, text="(Your matrix is").grid(
                row=2, column=self.cols*2+1)
            Label(self.frame_inverse_output, text="not invertible!)").grid(
                row=3, column=self.cols*2+1)
    ...
    def output_matrix(self):
        ...
        # display user input
        Label(self.frame_inverse_output, text='Input:', font=(
            'arial', 10, 'bold')).grid(row=1, column=1)
        for i in range(self.rows):
            for j in range(self.cols):
                Label(self.frame_inverse_output, text=self.matrix[i][j], bd=5).grid(
                    row=i+2, column=j+2)

        # display output
        if self.cols <= self.rows:
            Label(self.frame_inverse_output, text='Output:', font=(
                'arial', 10, 'bold')).grid(row=1, column=self.cols*2)
            for i in range(self.rows):
                for j in range(self.cols):
                    Label(self.frame_inverse_output, text=self.compute_inverse()[
                          i], bd=5).grid(row=i+2, column=self.cols*2+1)
    ...
    def input_matrix(self):
        ...
        Label(self.gui_inverse_input, text="Enter matrix:",
        ...
        for i in range(self.rows):
            ...
            for j in range(self.cols):
                ...
                if i == 1:
                    Label(self.gui_inverse_input, text=alphabet[j]).grid(
                        row=1, column=j+2)
                ...
                entries[i].append(
                    Entry(self.gui_inverse_input, textvariable=text_var[i][j], width=3))

                # display entry
                entries[i][j].grid(row=i+2, column=j+2)

                # for row indications
                Label(self.gui_inverse_input, text=i +
                      1).grid(row=i+2, column=1, sticky='e')
        ...
        Button(self.gui_inverse_input, text="Enter", width=8,
               command=get_mat).grid(row=self.m_height.get()+10, column=1)
    ...
    def __init__(self):
        ...
        Label(self.gui_inverse_menu, text='Enter matrix length:').grid(
            row=3, column=1)
        Label(self.gui_inverse_menu, text='Enter matrix height:').grid(
            row=4, column=1)
        ...
        self.m_length.set(2)
        self.m_height.set(2)
        OptionMenu(self.gui_inverse_menu, self.m_length,
                   *range(2, 11)).grid(row=3, column=2)
        OptionMenu(self.gui_inverse_menu, self.m_height,
                   *range(2, 11)).grid(row=4, column=2)

        Button(self.gui_inverse_menu, text='Enter', padx=16, pady=5,
               command=self.input_matrix).grid(row=5, column=2)
